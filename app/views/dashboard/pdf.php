...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    body { margin: 0; font-family: DejaVu Sans, sans-serif; font-size: 10px; color: <?= $colorText ?>; }
    .header { border: 1px solid <?= $colorGray ?>; border-radius: 10px; padding: 10px 12px; margin-bottom: 10px; }
    ...
    .logo { max-width: 100%; height: auto; max-height: 38px; margin-bottom: 6px; }
    .title { font-size: 16px; font-weight: bold; margin: 0 0 3px 0; }
    .muted { color: <?= $colorMuted ?>; }
    .badge { display: inline-block; padding: 2px 7px; border-radius: 999px; font-size: 9px; border: 1px solid <?= $colorGray ?>; background: #f9fafb; color: #374151; }
    .report-footer { position: fixed; bottom: -10mm; left: 0; right: 0; font-size: 9px; color: <?= $colorMuted ?>; text-align: center; }
    ...
    .card { border: 1px solid <?= $colorGray ?>; border-radius: 10px; padding: 10px 12px; height: 100%; }
    .k { font-size: 9px; color: <?= $colorMuted ?>; text-transform: uppercase; letter-spacing: 0.08em; }
    .v { font-size: 22px; font-weight: bold; color: <?= $colorBrown ?>; margin-top: 6px; }
    .sub { font-size: 10px; color: <?= $colorMuted ?>; margin-top: 4px; }
    .bar { height: 10px; border-radius: 999px; background: <?= $colorGray ?>; overflow: hidden; margin-top: 8px; }
    .bar > span { display: block; height: 10px; background: <?= $colorRed ?>; }
  ...
  <div class="header">
    ...
    <div style="overflow:hidden;">
      ...
      <?php if (!empty($branding['header_subtitle'])): ?>
        <div class="muted" style="margin-bottom: 3px;"><?= htmlspecialchars((string)$branding['header_subtitle'], ENT_QUOTES, 'UTF-8') ?></div>
      <?php endif; ?>
      <div class="muted">
          ...
          <table style="width:100%; border-collapse:collapse; margin-top:6px;">
            <tr>
              <td style="width:50%;">
                ...
                <div style="font-size:18px; font-weight:bold; color: <?= $colorBrown ?>;"><?= (int)($tre['planejados'] ?? 0) ?></div>
              </td>
              <td style="width:50%;">
                <div class="mini-title">Realizados</div>
                <div style="font-size:18px; font-weight:bold; color: <?= $colorBrown ?>;"><?= (int)($tre['realizados'] ?? 0) ?></div>
              </td>
            </tr>
          </table>
