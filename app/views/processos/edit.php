...
<div class="p-6 max-w-3xl">
  <div class="flex justify-between items-center mb-4">
    <h1 class="text-2xl font-bold"><?= htmlspecialchars($pageTitle ?? 'Editar Processo') ?></h1>
  ...
  <?php else: ?>
    <div class="bg-white shadow rounded p-4">
      <form method="post" action="index.php?route=processos/update" class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="md:col-span-2">
            ...
            <input class="border rounded p-2 w-full" type="text" name="nome" value="<?= htmlspecialchars((string)$item['nome']) ?>" required />
          </div>
          <div class="md:col-span-2">
            <label class="block text-sm">Descrição</label>
            <textarea class="border rounded p-2 w-full" name="descricao" rows="3"><?= htmlspecialchars((string)($item['descricao'] ?? '')) ?></textarea>
          </div>
          <div class="md:col-span-2">
            <label class="block text-sm">Responsável</label>
            <input class="border rounded p-2 w-full" type="text" name="responsavel" value="<?= htmlspecialchars((string)($item['responsavel'] ?? '')) ?>" />
          </div>
          <div>
            <label class="block text-sm">Data início</label>
            <input class="border rounded p-2 w-full" type="date" name="data_inicio" value="<?= htmlspecialchars((string)($item['data_inicio'] ?? '')) ?>" required />
          </div>
          <div>
            <label class="block text-sm">Data fim</label>
            <input class="border rounded p-2 w-full" type="date" name="data_fim" value="<?= htmlspecialchars((string)($item['data_fim'] ?? '')) ?>" />
          </div>
          <div class="md:col-span-2">
