...
<div class="p-6">
  <div class="flex justify-between items-center mb-4">
    <h1 class="text-2xl font-bold"><?= htmlspecialchars($pageTitle ?? 'Coaching') ?></h1>
    <div class="flex items-center gap-2">
      <a class="icon-btn icon-btn--primary icon-btn--lg" href="index.php?route=coaching/create" title="Novo coaching" aria-label="Novo coaching"><span data-feather="plus"></span></a>
  ...
  <form method="get" action="index.php" class="mb-4 flex flex-wrap items-end gap-2">
    <input type="hidden" name="route" value="coaching/index" />
    <div>
  ...
  <div class="bg-white shadow rounded overflow-x-auto">
    <table class="min-w-full text-sm">
      ...
      <tbody>
        <?php if (empty($items)): ?>
          <tr><td class="p-4 text-center text-gray-500" colspan="6">Nenhum coaching cadastrado.</td></tr>
        <?php endif; ?>
        <?php foreach ($items as $i): ?>
          <tr class="border-b">
            <td class="p-3">
              <div class="font-medium"><?= htmlspecialchars((string)($i['titulo'] ?? '')) ?></div>
              <?php if (!empty($i['observacoes'])): ?><div class="text-xs text-gray-500 mt-1"><?= htmlspecialchars((string)$i['observacoes']) ?></div><?php endif; ?>
            </td>
            <td class="p-3"><?= htmlspecialchars((string)($i['cliente'] ?? '')) ?></td>
            <td class="p-3 whitespace-nowrap"><?= htmlspecialchars((string)($i['data_inicio'] ?? '')) ?></td>
            <td class="p-3"><?= htmlspecialchars((string)($i['coach'] ?? '')) ?></td>
            <td class="p-3"><?= htmlspecialchars((string)($i['status'] ?? '')) ?></td>
            <td class="p-3 whitespace-nowrap">
              <a class="text-brand-pink icon-action" href="index.php?route=coaching/edit&id=<?= (int)$i['id'] ?>" title="Editar" aria-label="Editar"><span data-feather="edit"></span></a>
              <form method="post" action="index.php?route=coaching/finalizar" class="inline">
              ...
              <a class="text-brand-brown icon-action ml-2" href="index.php?route=coaching/delete&id=<?= (int)$i['id'] ?>" title="Excluir" aria-label="Excluir" onclick="return confirm('Excluir este registro?');"><span data-feather="trash-2"></span></a>
            </td>
          </tr>
        <?php endforeach; ?>
      </tbody>
