...
<div class="p-6 max-w-3xl">
  <div class="flex justify-between items-center mb-4">
    <h1 class="text-2xl font-bold"><?= htmlspecialchars($pageTitle ?? 'Editar Tarefa') ?></h1>
  ...
  <?php else: ?>
    <div class="bg-white shadow rounded p-4">
      <form method="post" action="index.php?route=tarefas/update" class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select class="border rounded p-2 w-full" name="prioridade">
              <?php foreach (['baixa' => 'Baixa', 'media' => 'Média', 'alta' => 'Alta'] as $val => $lbl): ?>
                <option value="<?= $val ?>" <?= ((string)($item['prioridade'] ?? 'media')) === $val ? 'selected' : '' ?>><?= $lbl ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div>
