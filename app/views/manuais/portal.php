...
<body class="bg-gray-100 text-gray-900">
  <div class="max-w-6xl mx-auto p-4 md:p-6">
    ...
    <div class="bg-white shadow rounded p-5 mb-4">
      <h1 class="text-2xl font-bold">Portal de Manuais</h1>
      <p class="text-sm text-gray-600 mt-1">
        Empresa: <strong><?= htmlspecialchars((string)($empresa['nome_empresa'] ?? 'Cliente')) ?></strong>
      </p>
      <p class="text-sm text-gray-600">Acesso restrito aos manuais autorizados para esta empresa.</p>
    </div>
    ...
    <div class="bg-white shadow rounded overflow-x-auto">
      <div class="px-4 py-3 border-b text-sm text-gray-600">Total de manuais: <?= (int)$total ?></div>
      <table class="min-w-full">
        ...
        <tbody>
          <?php if (empty($items)): ?>
            <tr><td colspan="6" class="p-4 text-sm text-gray-600">Nenhum manual encontrado.</td></tr>
          <?php else: ?>
            <?php foreach ($items as $manual): ?>
              <tr class="border-b">
                ...
                <td class="p-3">
                  <a class="px-3 py-2 rounded bg-red-600 text-white inline-block" href="<?= $basePath ?>/manuais/download/<?= (int)$manual['id'] ?>">Baixar</a>
                </td>
              </tr>
            <?php endforeach; ?>
          <?php endif; ?>
