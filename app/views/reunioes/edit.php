...
<div class="p-6 max-w-3xl">
  <div class="flex justify-between items-center mb-4">
    <h1 class="text-2xl font-bold"><?= htmlspecialchars($pageTitle ?? 'Editar Reunião') ?></h1>
  ...
  <?php else: ?>
    <div class="bg-white shadow rounded p-4">
      <form method="post" action="index.php?route=reunioes/update" class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="md:col-span-2">
            ...
            <input class="border rounded p-2 w-full" type="text" name="local" value="<?= htmlspecialchars((string)($item['local'] ?? '')) ?>" />
          </div>
          <div class="md:col-span-2">
            <label class="block text-sm">Pauta</label>
            <textarea class="border rounded p-2 w-full" name="pauta" rows="3"><?= htmlspecialchars((string)($item['pauta'] ?? '')) ?></textarea>
          </div>
          <div>
