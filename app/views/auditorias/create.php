...
<div class="p-6 max-w-6xl">
    ...
    <form method="post" action="index.php?route=auditorias/store" id="auditoriaForm" class="bg-white shadow rounded p-4 space-y-4">
        ...
        <div class="text-xs text-gray-600">Anexos são incluídos somente durante a auditoria das questões.</div>
    ...
    (function(){
        ...
        form?.addEventListener('submit', (e)=>{
            ...
            for (let i = 0; i < questoes.length; i++) {
                ...
                if (!hasResponsavelPreenchido(i)) {
                    e.preventDefault();
                    alert(`Questão ${i + 1}: selecione pelo menos 1 responsável.`);
                    return;
                }
            }
            if (!questoes.some((_, idx)=>hasResponsavelPreenchido(idx))) {
                e.preventDefault();
