<?php use App\Core\Security; /** @var array $clientes */ /** @var array $responsaveis */ /** @var array $setores */ /** @var int $selectedCliente */ /** @var int $selectedSetor */ /** @var array $values */ /** @var array $errors */ ?>
<div class="p-6 max-w-5xl">
    <h1 class="text-2xl font-bold mb-4">Cadastrar Nova Auditoria</h1>
    <form method="post" action="index.php?route=auditorias/store" id="auditoriaForm" class="bg-white shadow rounded p-4 space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
            ...
            <div class="md:col-span-4">
                ...
                <select name="responsavel_id" id="responsavelSelect" class="border rounded p-2 w-full" required>
                    ...
                    <?php foreach ($responsaveis as $c): ?>
                        <option value="<?= (int)$c['id'] ?>" <?= ((int)($values['responsavel_id'] ?? 0) === (int)$c['id']) ? 'selected' : '' ?>><?= htmlspecialchars($c['nome']) ?></option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="md:col-span-12">
                ...
                <textarea name="pergunta" id="perguntaField" class="border rounded p-2 w-full" rows="3" minlength="10" maxlength="500" required><?= htmlspecialchars($values['pergunta'] ?? '') ?></textarea>
            ...
            <div class="md:col-span-12">
                ...
                <textarea name="objetivo" id="objetivoField" class="border rounded p-2 w-full" rows="4" minlength="20" required><?= htmlspecialchars($values['objetivo'] ?? '') ?></textarea>
            ...
            <div class="md:col-span-12">
                ...
                <textarea name="referencia_esperada" class="border rounded p-2 w-full" rows="4" required><?= htmlspecialchars($values['referencia_esperada'] ?? '') ?></textarea>
...
<script>
    (function(){
        ...
        clienteSelect?.addEventListener('change', loadSetores);
        ...
        form?.addEventListener('submit', (e)=>{
            ...
            const p = (perguntaField.value || '').trim();
            const o = (objetivoField.value || '').trim();
            if (!/^\d{2}\/\d{2}\/\d{4}$/.test(d)) {
                e.preventDefault();
                alert('Data inválida. Use DD/MM/YYYY.');
                return;
            }
            if (p.length < 10 || p.length > 500) {
                e.preventDefault();
                alert('A pergunta deve ter entre 10 e 500 caracteres.');
                return;
            }
            if (o.length < 20) {
                e.preventDefault();
                alert('O objetivo deve ter no mínimo 20 caracteres.');
                return;
            }
